...
- **DigiLocker** and **Account Aggregator** — scaffolded, not live. `DigiLockerService`/`DigiLockerController` (`/api/v2/sih/digilocker/*`) implement the full OAuth 2.0 authorization-code flow against DigiLocker's published Partner API — authorize-URL, single-use-state callback, token exchange, issued-document listing — and `AccountAggregatorService`/`AccountAggregatorController` (`/api/v2/sih/account-aggregator/*`) implement the consent request/status lifecycle against a TSP's REST API (e.g. Setu/Finvu, not the raw ReBIT network spec directly — see the class javadoc for why). Both refuse every entry point with **400** until real credentials are configured (`DIGILOCKER_CLIENT_ID`/`_SECRET`/`_REDIRECT_URI`, `ACCOUNT_AGGREGATOR_API_KEY`/`_BASE_URL`) — no DigiLocker Partner Portal or AA/FIU registration exists for this deployment, so none of this has been run against a real server. `AccountAggregatorService.fetchFinancialData()` additionally throws outright even once configured: decrypting fetched FI data needs the AA spec's ECDH/AES-GCM exchange, deliberately not implemented rather than guessed at with no real counterparty to validate against. `VerificationMode.DIGILOCKER`/`.ACCOUNT_AGGREGATOR` stay `isAvailable() == false` regardless — flipping that is a manual step for once someone has actually run this against a live sandbox, not something that follows from the code existing.
- **UIDAI signature verification** on offline eKYC — extraction/masking/storage all work today, and so does the real XML-DSig signature check (`AadhaarOfflineEkycExtractor`) against whatever certificate is configured; only a real production `UIDAI_CERT_PATH` is missing from this deployment.
